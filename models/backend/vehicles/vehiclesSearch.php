<!DOCTYPE html>
    ...
    // $query = "SELECT * FROM vehicle WHERE name LIKE :searchContent";
    $query = "SELECT v.*, b.name as brandName, vt.name as vehicleTypeName
                    ...
                    WHERE v.name LIKE :searchContent" ;
            ...
            <table class="w-full text-sm text-left rtl:text-right text-gray-50 dark:text-gray-400 rounded">
                ...
                <?php while ($row = $stmt->fetch()) { ?>
                    ...
                    <tr class="bg-white border-b dark:bg-gray-800 dark:border-gray-700">
                                ...
                                <td class="py-8 flex justify-center items-center">
                                    <?php 
                                        if( $row->image === "" ) {
                                            ...
                                            ?>
                                            <img src="../../assets/vehiclesImages/<?php echo $row->image; ?>" class="w-20  md:w-10 max-w-full max-h-full">
                                            <?php
                                        }
                                    ?>
                                </td>
                                ...
                                <td class="px-6 py-2 font-bold text-gray-900 dark:text-white">
                                    <?php echo $row->vehicleTypeName; ?>
                                </td>
                                <td class="px-6 py-2 font-bold text-gray-900 dark:text-white">
                                    <?php echo $row->modelYear; ?>
                                </td>
                                <td class="px-6 py-2 font-bold text-gray-900 dark:text-white">
                                    <?php echo $row->costPerDay; ?> DA
                                </td>
                                <td class="px-6 py-2 font-bold <?php echo ($row->vehicleStatus == 'Available') ? 'text-green-500' : (($row->vehicleStatus == 'Not Available') ? 'text-red-500' : 'text-blue-500'); ?>">
                                ...
                                <td class="px-6 py-2">
                                    <a href="../vehicles/editVehicle.php?id=<?php echo $row->vehicleID ?>">
